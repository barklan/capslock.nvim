...
local function checkString(name, s)
    ...
    if S.len(s) > ustring.maxStringLength then
        local msg = S.format(
            "bad argument #1 to '%s' (string is longer than %d bytes)",
            name,
            ustring.maxStringLength
        )
        error(msg, 3)
    end
end
...
local function checkPattern(name, pattern)
    ...
    if S.len(pattern) > ustring.maxPatternLength then
        local msg = S.format(
            "bad argument #2 to '%s' (pattern is longer than %d bytes)",
            name,
            ustring.maxPatternLength
        )
        error(msg, 3)
    end
end
...
local function internalChar(t, s, e)
    ...
    for i = s, e do
        ...
        if v < 0 or v > 0x10ffff then
            error(
                S.format("bad argument #%d to 'char' (value out of range)", i),
                2
            )
        elseif v < 0x80 then
            ret[#ret + 1] = v
...
function ustring.upper(s)
    checkString("upper", s)
    local map = require("capslock.ustring.upper")
...
function ustring.lower(s)
    checkString("lower", s)
    local map = require("capslock.ustring.lower")
    local ret = S.gsub(s, "([^\128-\191][\128-\191]*)", map)
    return ret
end
...
local function find(s, cps, rawpat, pattern, init, noAnchor)
    local charsets = require("capslock.ustring.charsets")
    local anchor = false
    ...
    local function getcapt(n, err, errl)
        ...
        elseif type(captures[n]) == "table" then
            ...
            return sub(s, cps, captures[n][1], captures[n][2]),
                captures[n][2] - captures[n][1] + 1
        else
        ...
        elseif c == 0x25 then -- '%'
            ...
            elseif c == 0x66 then -- '%f': frontier pattern match
                if pattern.codepoints[pp + 2] ~= 0x5b then
                    error(
                        "missing '[' after %f in pattern at pattern character "
                            .. pp,
                        3
                    )
                end
    ...
    parse_charset = function(pp)
        ...
        repeat
            ...
            if not ep then
                error(
                    "Missing close-bracket for character set beginning at pattern character "
                        .. pp,
                    3
                )
            end
        ...
        while true do
            ...
            elseif c == 0x25 then -- '%'
                ...
                pp = pp + 2
            elseif
                pattern.codepoints[pp + 1] == 0x2d
                and pattern.codepoints[pp + 2]
                and pattern.codepoints[pp + 2] ~= 0x5d
            then -- '-' followed by another char (not ']'), it's a range
    ...
    repeat
        ...
        if ep then
            for i = 1, ncapt do
                captures[i] = getcapt(
                    i,
                    "Unclosed capture beginning at pattern character "
                        .. captparen[i],
                    2
                )
            end
...
local function patternIsSimple(pattern)
    ...
    return not (
            S.find(pattern, "[\128-\255]")
            or S.find(pattern, "%[%^")
            or S.find(pattern, "%%[acdlpsuwxACDLPSUWXZ]")
            or S.find(pattern, "%.[^*+-]")
            or S.find(pattern, "%.$")
            or S.find(pattern, "()", 1, true)
            or pattern == ""
            or findWithPcall("", pattern)
        )
end
...
local function internalDecompose(cps, decomp)
    local cp = {}
    local normal = require("capslock.ustring.normalization_data")
