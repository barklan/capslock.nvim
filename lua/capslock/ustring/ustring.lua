...
local function checkType(name, argidx, arg, expecttype, nilok)
	...
	if type(arg) ~= expecttype then
		local msg = S.format("bad argument #%d to '%s' (%s expected, got %s)", argidx, name, expecttype, type(arg))
		error(msg, 3)
	end
end
...
local function checkString(name, s)
	...
	if type(s) ~= "string" then
		local msg = S.format("bad argument #1 to '%s' (string expected, got %s)", name, type(s))
		error(msg, 3)
	end
	if S.len(s) > ustring.maxStringLength then
		local msg = S.format("bad argument #1 to '%s' (string is longer than %d bytes)", name, ustring.maxStringLength)
		error(msg, 3)
	end
end
...
local function checkPattern(name, pattern)
	...
	if type(pattern) ~= "string" then
		local msg = S.format("bad argument #2 to '%s' (string expected, got %s)", name, type(pattern))
		error(msg, 3)
	end
